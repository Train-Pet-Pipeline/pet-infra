...
## 10. Phase 2 Data & Annotation 运行时（pet-data 1.2.0, pet-annotation 2.0.0）

Phase 2 在 Phase 1 Foundation 之上把 `pet-data` 和 `pet-annotation` 接入插件体系，引入 modality 维度，并将 annotation 体系升级到 4 范式（llm / classifier / rule / human），对应 `pet-schema v2.1.0` 的四表结构。

### 10.1 Modality 判别字段

所有 Sample / Annotation 载荷都带 `modality` 字段（`Literal["vision", "audio", "sensor", "multimodal"]`）。Pydantic v2 discriminated union 以它为判别键，消费方 `pet_schema.samples.Sample` / `pet_schema.annotations.Annotation` 在反序列化时自动路由到正确的子类（`VisionSample` / `AudioSample` / `LLMAnnotation` / `ClassifierAnnotation` / `RuleAnnotation` / `HumanAnnotation`）。
...
### 10.3 Phase 2 注册的 dataset 插件（4 范式）

pet-annotation v2.0.0 将 annotation 按生产方式拆为 4 个独立插件，对应 `pet-schema v2.1.0` 的四张表（`llm_annotations` / `classifier_annotations` / `rule_annotations` / `human_annotations`）：

| Registry key                        | 来源仓库         | 产出 Pydantic 类型        | DB 表                    |
|-------------------------------------|------------------|---------------------------|--------------------------|
| `pet_data.vision_frames`            | pet-data         | `VisionSample`            | `frames`                 |
| `pet_data.audio_clips`              | pet-data         | `AudioSample`             | `audio_samples`          |
| `pet_annotation.llm`                | pet-annotation   | `LLMAnnotation`           | `llm_annotations`        |
| `pet_annotation.classifier`         | pet-annotation   | `ClassifierAnnotation`    | `classifier_annotations` |
| `pet_annotation.rule`               | pet-annotation   | `RuleAnnotation`          | `rule_annotations`       |
| `pet_annotation.human`              | pet-annotation   | `HumanAnnotation`         | `human_annotations`      |
...
下游消费方（pet-train、pet-annotation、pet-eval）只能依赖 `VisionSample` / `AudioSample` / `LLMAnnotation` / `ClassifierAnnotation` / `RuleAnnotation` / `HumanAnnotation`，禁止 `from pet_data.storage.store import FrameRecord` 这样的跨仓内部类型引用。对应地，pet-annotation v2.0.0 拆成四个独立 adapter，row→pydantic 由各自的 `pet_annotation.<paradigm>.adapter` 完成。
...
- pet-data：`002_add_modality_storage_uri.py`（为 `frames` 补 `modality` / `storage_uri` / `frame_width` / `frame_height` / `brightness_score`），`003_add_audio_samples.py`（新建 `audio_samples` 表）
- pet-annotation：`002_add_modality.sql`（为旧 `annotations` 补 `modality` 和 `storage_uri`），`003_create_llm_annotations.sql`，`004_create_classifier_annotations.sql`，`005_create_rule_annotations.sql`，`006_create_human_annotations.sql`（新建四张范式表）

Spec §7.3 原本提到 "Alembic 迁移"，但两仓从 v1 起就使用上述手写方案，Phase 2 保留现状（YAGNI）。后续若要引入 Alembic，须作为独立基础设施项目单独排期。

### 10.7 Phase 2 smoke recipe

`pet-infra/recipes/pet_data_ingest_smoke.yaml` 是跨仓 smoke，通过 `ref_type: dataset / ref_value: pet_data.vision_frames` 证明 dataset 插件分发可用，以 `pet_infra.noop_evaluator` 作为不做真正计算的 stage component 让 `preflight` 通过。CI 侧 `plugin-discovery.yml` 会安装 `pet-data@v1.2.0` + `pet-annotation@v2.0.0` 并断言 6 个 Phase 2 dataset key（2 个 pet-data + 4 个 pet-annotation 范式）全部到位。
...
All 10 repos (pet-schema / pet-data / pet-annotation / pet-train / pet-eval / pet-quantize / pet-ota / pet-infra / pet-id / pet-demo) are licensed under Business Source License 1.1 (source-available). See each repo's `LICENSE`. Change Date 2030-04-22 → Apache License, Version 2.0.
