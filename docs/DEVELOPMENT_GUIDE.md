...
- 帧长：25ms，帧移：10ms
- MFCC：40 维系数（含 delta + delta-delta = 120 维输入）
- 每次推理输入：1 秒音频片段（100 帧）

**模型架构：**

```
输入: (batch, 100, 120)  ← 100帧 × 120维特征
  ↓
MobileNetV3-Small（去掉最后分类头，保留特征提取部分）
  ↓
GlobalAveragePooling2D
  ↓
Linear(960, 256) + ReLU + Dropout(0.3)
  ↓
Linear(256, 4)  ← 4类输出
  ↓
Softmax → distribution: {chewing, drinking, vocalization, pre_vomit}
```

**训练数据要求：**
- 每类最少 300 段 5 秒音频（实际有效声音，不含纯背景）
- 来源：宠物音频公开数据集（ESC-50 动物子集）+ 真实设备麦克风录制
- 录制必须用实际产品设备，不能用手机代替（麦克风频响曲线不同）
...
└── Makefile
补上vendor/ 放LLaMA-Factory 源码
```

注意在vendor/放LLaMA-Factory 源码
...
def compute_kl_distillation_loss(
    student_logits: torch.Tensor,   # 2B 模型输出
    teacher_logits: torch.Tensor,   # 72B 模型缓存输出（预计算）
    temperature: float = 2.0,
    lambda_kl: float = 0.1          # 从 params.yaml 读取
) -> torch.Tensor:
    """
    软标签蒸馏。temperature=2.0 软化分布，让小模型学到不确定性。
    lambda_kl=0.1 是起点，通过消融实验 (sft_kl_lambda_*.yaml) 调整。
    """
    student_log_prob = F.log_softmax(student_logits / temperature, dim=-1)
    teacher_prob = F.softmax(teacher_logits / temperature, dim=-1)
    kl = F.kl_div(student_log_prob, teacher_prob, reduction='batchmean')
    return lambda_kl * (temperature ** 2) * kl  # temperature^2 补偿梯度缩放
```
