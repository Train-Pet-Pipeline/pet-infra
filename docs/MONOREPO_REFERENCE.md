...
理解 9 个仓如何协作，最有效的切入点不是任何一个仓的内部实现，而是**数据在仓与仓之间
的流转形态**——什么类型的对象从哪里出来，被谁消费，在中间被增量补哪些字段。这一节给出
两张视图：一张是"从原始视频帧到端侧设备"的全链路 flowchart（每一步标出输入/输出类型
和经手仓库），另一张是"`ModelCard` 在生命周期里如何被多个仓接力填字段"的状态机。

第一张图展示的是 stage-level 的数据流——pet-data 从外部源拉视频/图片产出 `FrameRecord`
进 SQLite、pet-annotation 给 frame 配上 `Annotation` 后导出 LLaMA-Factory JSONL、
pet-train 跑 SFT/DPO 写出第一份 `ModelCard`、pet-eval 跑 metrics 把 GateCheck 结果回填、
pet-quantize 把 checkpoint 转成 `EdgeArtifact` 列表、pet-ota 把签名 tarball 推到设备。
看这张图主要建立"谁产生什么、谁消费什么"的全局印象，后面 §3 各仓深读时可以随时回到这里
对照"我现在在改的这个仓位于流水线哪一段"。

第二张图聚焦 `ModelCard` 这一个对象——它是整条流水线里跨阶段流动最远的契约，从 trainer
plugin 在 `run()` 完成时第一次创建（填 identity / reproducibility / artifact 字段），
到 evaluator plugin 把 `metrics` 和 `gate_status` 写回，再到 quantize plugin append
`edge_artifacts`，最后 OTA plugin append `deployment_history` 与可选的 `hardware_validation`。
每一步都用 Pydantic `model_copy(update=...)` 做函数式追加而不是 mutate，让 trace 时
能精确看到"这张卡在 stage N 之前长什么样"。理解了这张状态机，就理解了为什么 ModelCard
的 `extra="forbid"` 是必要约束——如果允许任意 plugin 随手加字段，跨阶段契约的可读性
立即崩塌。
...
pet-schema 是整个生态的数据类型定义层，类比 protobuf 的 `.proto` 文件——它本身不执行
任何业务逻辑、不做 I/O、不注册 plugin，只承担一件事：用 Pydantic 定义所有跨仓库流动的
数据结构（`Annotation`、`ModelCard`、`ExperimentRecipe`、`EdgeArtifact` 等），并在
VLM 输出层提供一个语义校验入口（`validate_output()`）。下游 7 个仓全部 `import pet_schema`
来 model_validate / dump，pet-schema 自己反过来不依赖任何 pet-* 包，是依赖图的真正根节点。

仓内代码集中在 `src/pet_schema/` 一层之下：`annotations.py` 装着 4 范式标注的判别联合
（LLM / 分类器 / 规则 / 人审，由 Pydantic Discriminator 路由），`model_card.py` 定义
了那个串起整条流水线的 `ModelCard` 契约，`recipe.py` 把 YAML recipe 解析为 `ExperimentRecipe`
并提供 `to_dag()` 让 pet-infra Orchestrator 拿 networkx 调度，`training_samples.py`
固化了 LLaMA-Factory 期望的 JSONL 元素（SFT/ShareGPT/DPO 三种），`validator.py` 用
`versions/v1.0/schema.json` 配 Python 业务规则做 VLM 输出双层校验。tests/ 里几乎全是
round-trip 和 `extra="forbid"` 拒绝行为的回归用例——任何 PR 改了 schema 都要在这一层
留下化石证据。

新工程师上手时只需要记住四个名字：`Annotation` 是所有标注的类型路由入口，`ModelCard`
是训练/量化/OTA 三段都要读写的状态契约（`extra="forbid"` 让任意"动态加字段"在运行时
立即抛 ValidationError），`ExperimentRecipe.to_dag()` 是 YAML 到执行图的转换点，
`validate_output()` 是 VLM 输出进入 DB 之前的最后一道闸门。其余类型（`EdgeArtifact`、
`ResourceSpec`、`HardwareValidation`、`DeploymentStatus` 等）是这四个核心契约的字段
类型，本身不做协议工作，按需阅读即可。

**何时改 pet-schema、何时不该改**

需要改 pet-schema 的场景非常窄：增加一个新的标注生产者类型（4 范式之外）、`ModelCard`
新增字段（比如 trainer 想记录某个新指标或 artifact 类别）、VLM JSON Schema 加字段或
业务规则、Recipe DSL 新增 stage 类型。流程同样窄：PR + ≥2 reviewer approve + bump
版本号 + `compatibility_matrix.yaml` 同步下游 pin。这种重流程不是过度工程，而是 F018/F019
两次 retro 的直接产物——pet-schema 漂移过一次几乎让 pet-annotation v2.2.1 把 F001
回归回去，所以现在严格管。如果你只是改 evaluator 内部字段名或 pet-train 的 wrapper
配置项，那是消费方仓的内部细节，不要扩散到 pet-schema 层。
...
pet-infra 是这条流水线的执行底座，承担 Orchestrator 的角色。它本身不知道怎么训练模型、
怎么评估、怎么量化，只知道一件事：从 `ExperimentRecipe` 拿到 stage 列表，根据 DAG 顺序
依次从 Registry 取出对应 plugin、按 config 实例化、调用 `.run(input_card, recipe)`。
这个"thin orchestrator"的赌注让所有业务知识都住在 sibling 仓——pet-train 来贡献 trainer、
pet-eval 来贡献 evaluator、pet-quantize 来贡献 converter——pet-infra 永远不 `import pet_train`。
回报是三个具体的工程性质：新增 trainer 时 pet-infra 的代码 0 改动；把实验追踪器从 W&B
换到 ClearML 时下游 7 仓没人感知到；任何 stage 都能被 `pet replay` 确定性重放，因为
Orchestrator 自己不含任何 stage-specific 的逻辑分支。

代码组织围绕这条 stage-execution path 展开：`src/pet_infra/registry.py` 定义 7 个 mmengine
Registry（TRAINERS / EVALUATORS / CONVERTERS / DATASETS / STORAGE / OTA / METRICS）作为
所有 plugin 的中央存放点；`orchestrator/runner.py` 的 `pet_run()` 是真正的执行循环，
负责按 DAG 顺序跑 stage、把每个 stage 产出的 `ModelCard` 喂给下一个 stage、把 metrics
转发给 ExperimentLogger；`replay.py` 是确定性重放路径，靠 `hydra_config_sha` 做硬比对、
靠 `git_shas` 做软漂移告警；`experiment_logger/` 装着 ClearML / offline / null 三个 Logger
backend，由 stack tag 决定走哪一个；`storage/` 提供 file/S3/HTTP 三个 backend 让 plugin
不需要关心 artifact 落地在哪。CLI 入口在 `cli/` 目录，命令行 `pet validate` / `pet run` /
`pet replay` 都是从这里分派到上述模块的薄包装。

新人最先要建立的认知是 7 个 Registry 的角色划分、`pet_run()` 那一段几十行的执行循环、
还有 ModelCard 在 stage 之间如何 `model_copy(update=...)` 流转——这三点掌握了，整个
pet-infra 就可读。其余文件（hooks、storage backend 选择策略、CLI 子命令路由）多半是
配套机制，遇到具体问题再去翻。如果你来 pet-infra 是为了改 Orchestrator 行为本身（比如
加一种新的 stage 调度策略），那基本上只动 `orchestrator/runner.py` + `replay.py`；如果
你只是想新增一种 plugin 类型，那不该改 pet-infra，去对应的 sibling 仓贡献。
...
pet-data 是整条流水线的源头仓，负责把原始视频和图片转化为可标注的 frame 记录。它的工作
是"看似无聊但极其重要"的那一类——下载、解帧、感知 hash 去重、亮度/模糊质量检测、写入
SQLite——之所以独立成仓不是出于代码量考虑，而是为了把"跳过 dedup"这种诱惑物理隔离：
混在 train 仓里时跳过 dedup 只是注释掉一行函数调用，独立成仓后跳过就意味着跳过一整个
仓的产物，是显眼的事故。

代码主线沿着 `BaseSource → FrameStore` 这条 ingest pipeline 展开。`src/pet_data/sources/`
存放 6 个 ingester 实现（`oxford_pet.py`、`coco_pet.py`、`youtube.py`、`community.py`、
`hospital.py`、`local_dir.py`、`selfshot.py`），每个继承 `base.py::BaseSource` 并填两个
抽象方法（`download()` 产 RawItem、`validate_metadata()` 校验必填字段），其余下载→抽帧→
去重→质检→落库的流水骨架由 `BaseSource.ingest()` template method 统一处理。
`processing/` 目录里 `dedup.py` 用 perceptual hash + 汉明距离判重、`quality_filter.py`
用 cv2 算亮度和模糊度、`anomaly.py` 用 autoencoder 给异常分；这三个组件在 ingest 流程
中被顺序调用，参数全部从 params.yaml 读，不硬编码。`storage/store.py` 的 `FrameStore`
是唯一允许写 sqlite 的入口（dataset plugin 的只读 streaming read 是有意为之的例外），
打开连接时立即跑全部 migration（`migrations/002_*.py` 起依次幂等执行），让"打开数据库"
和"schema 最新"不可分。`weak_supervision/` 装着 Snorkel-style 弱监督打标 LF；
`augmentation/` 是 albumentations 包装的数据增强；`datasets/vision_frames.py` 是给
pet-train 当 DATASETS plugin 用的 streaming 迭代器。

进入这个仓最先要建立的认知是 ingester_name 与 default_provenance 的概念分离——前者是
实现身份（"这条记录是 oxford_pet 这个代码模块产生的"），后者是合规分类（pet-schema
`SourceType` 枚举值之一，DB 有 CHECK constraint）。F1 finding 就是因为早期版本把这两
件事混成一个字段。其次是 `BaseSource.ingest()` 流水步骤的顺序——dedup 在 quality 之前
不是随手写的，是为了减少 quality 这一步的 CPU-bound 计算量。第三是 `FrameStore` 的
migration 幂等设计——`duplicate column name` 是预期路径不是异常路径，因为每次打开 DB
都跑全部 migration。

来 pet-data 改代码的典型场景：新增一种数据源（写一个新的 `BaseSource` 子类）、新增一种
质检规则（在 `processing/` 加文件）、新增一种弱监督 LF、迁移 frames 表 schema（写新的
`migrations/N_*.py` 而不是修旧文件——已提交的 migration 永不可变）。如果你想加一种新
的标注流程或训练流程，那不该改 pet-data，去对应的 sibling 仓。
...
pet-annotation 把"给一帧图打标签"这件事拆成 4 种生产者范式（LLM 调用、传统分类器、规则
引擎、人工审核），每种用对应的 Pydantic 子类描述（`LLMAnnotation` / `ClassifierAnnotation`
/ `RuleAnnotation` / `HumanAnnotation`），由 `AnnotationOrchestrator` 顺序调度。整个仓
独立存在的核心理由是：打标需要一套独立的状态机来追踪"每条 target 在每个 annotator 下
处于什么状态"——这套状态机和 pet-data 的数据状态、pet-train 的训练状态完全正交。
混进任何一个相邻仓都会模糊责任边界。pet-annotation 也刻意不直接写 pet-data 的 frames
表，pet-data 自留的 `annotation_status` 字段是给人工 QA 用的；pet-annotation 自己用
`annotation_targets` 表维护状态机，两套 status 独立演化互不污染。

主要代码住在 `src/pet_annotation/orchestrator.py` 和 `src/pet_annotation/annotators/`：
orchestrator 负责按 4 范式顺序拉一批 target、为每个 target 调对应 annotator、把结果
按 Pydantic discriminator 路由进 4 张范式表（`llm_annotations` / `classifier_annotations`
/ `rule_annotations` / `human_annotations`，而不是单一宽表）。`annotators/llm.py` 是
最重的一块，封装 Doubao / Qwen-VL API 调用、prompt 渲染、retry 策略、prompt_hash 缓存
（同一 prompt + 同一帧不重复调 API）；`annotators/classifier.py` 是传统 CV 分类器入口；
`annotators/rule.py` 装着规则引擎（关键词 + 简单 heuristics）；`annotators/human.py`
和 Label Studio 1.23 集成，session-auth 流程在 `labelstudio/session.py`。导出层在
`export/sft_dpo.py` 把范式表转成 LLaMA-Factory 期望的 JSONL（ShareGPT/SFT/DPO 三种），
是 pet-train 的输入；`export/jsonl_validator.py` 是 producer-side 一致性 guard，
F11 finding 之后强制启用，确保 export 出来的 JSONL 一定能被 pet-train consumer-side
validator 接住。质检相关的 `quality/` 目录有 inter-annotator agreement、distribution
drift detection、自动重审策略。

新人最先抓住的是"4 范式表"这个数据组织方式——为什么不是单一宽表？因为每种生产者的字段
结构差异很大（LLM 有 prompt_hash 和 prompt_version、classifier 有 class_probs、rule
有 matched_rules、human 有 reviewer_id 和 review_session_id），合并到一张宽表意味着
大量 nullable 列加 partial CHECK constraint，运行时反而更难校验完整性。其次是
`AnnotationOrchestrator` 那把 `asyncio.Lock` 守 sqlite3 connection 的细节——sqlite3
连接非线程安全，4 范式 orchestrator 并发跑各自 annotator（LLM 大量 API call 是 IO-bound）
时只能让一个写者进入。再就是 export 路径——pet-annotation 是 LLaMA-Factory JSONL 契约
的"生产者"，与 pet-train 的"消费者"validator 必须一致，每次 schema 改字段两边都得动。

来 pet-annotation 改代码的常见场景：加一种新 annotator（继承一个范式 base 写新文件
+ 注册）、改 export JSONL 格式（动 `export/sft_dpo.py` + 同步 pet-train consumer
validator + bump pet-schema）、改 quality 检查规则、调 prompt 模板（`prompts/` 目录
统一管 versioned 模板，`prompt_hash` 自动跟着变）。如果你想加新的训练范式，那是 pet-train
的事；如果你想加新数据源，那是 pet-data 的事——pet-annotation 只接已经在 pet-data 里
存好的 frame。
...
pet-train 是模型训练的"动力舱"，对接 LLaMA-Factory（VLM 的 SFT 与 DPO）和音频 CNN
（基于 PANNs）两条训练路线，把每一次训练完成的产物写成 `ModelCard` 交出去。它独立成仓
的根本原因是依赖隔离——`torch>=2.1` + `transformers` + LLaMA-Factory 的依赖体积让
pet-eval、pet-quantize、pet-ota 这些不需要训练能力的下游仓不可能安得起。pet-train 通过
3 个 entry-point plugin（`llamafactory_sft` / `llamafactory_dpo` / `audio_cnn_trainer`）
向 `pet_infra.registry.TRAINERS` 注册自己，pet-infra 永远不 `import pet_train`，这是
thin orchestrator 契约的体现。需要补充的是，pet-eval 在运行时 import `pet_train.audio.inference`
拿 PANNs 推理能力——这是整个生态唯一一处跨仓运行时 import，不是编译时依赖（`pet-train`
是 pet-eval 的 β peer-dep），保持单向流动避免循环。

代码主线在 `src/pet_train/plugins/`：`llamafactory_sft.py` 和 `llamafactory_dpo.py`
都是 LLaMA-Factory 的薄 wrapper，把 ExperimentRecipe 的 stage config 翻译成
LLaMA-Factory `run_exp(args=dict)` 期望的 dict 形式，启动训练，等结束后从 `all_results.json`
+ `trainer_state.json` 抓 metrics 填进 `ModelCard.metrics`（F022/F023 retro 的产物——
早期版本 `metrics={}` 硬编码，导致 `train_loss` 和 DPO `rewards/margins` 永远丢失）。
`audio.py` 是音频 CNN 训练的 plugin 入口，依赖 `audio/` 子目录里的 PANNs backbone
封装、数据增强、训练循环。`lineage.py` 提供 `collect_git_shas()` 让 trainer 把多仓 HEAD
sha 写进 `ModelCard.git_shas`——F024 retro 把这里从"硬编码 underscore key"修成"sibling
walk + hyphenated key"，pet-infra 的 replay 漂移检测才能真的工作。`vendor/LLaMA-Factory/`
是 git submodule 形式的 LLaMA-Factory 源码，不是 pip 装的——为了能 monkey-patch 内部
hook 把训练事件转给 ExperimentLogger。`tests/fixture_real/` 装着 fixture-real 测试，
跑真实的 8-step toy training 验证整条产出链路（DEV_GUIDE §11.8 retro guardrail 强制）。

新人最先要看的是 `LlamaFactorySFTTrainer.run()` 那几十行代码——它把整个训练 plugin 的
契约浓缩到一个函数：preflight 检查 data_path 存在、用 cfg 里的参数实例化 LF args dict、
调 `run_exp(args=...)` 跑训练、等结束后读 `output_dir/all_results.json` 拼 metrics、
用 `model_copy(update={...})` 写出新 ModelCard（注意 `checkpoint_uri` 不能含 `/adapter`
后缀——F025 retro，下游 inference/eval 加载会找不到）。其次是 `_collect_train_metrics`
这个辅助函数——SFT 抓 train_loss、DPO 走 trainer_state.json 反向遍历找 `rewards/*`
entry，两套不同 parsing 规则。再就是 `lineage.collect_git_shas` 的 sibling walk 逻辑——
为什么走 `parents[3]` 而不是 `parents[4]`：F024 root cause 就是这条路径上的 off-by-one。

来 pet-train 改代码的典型场景：加一个新 trainer plugin（写新文件继承 plugin 接口 + 在
`_register.py` 加注册）、改 LF wrapper 的参数翻译规则、改 metrics 抓取逻辑（动
`_collect_train_metrics` 注意同时改 SFT 和 DPO 两条分支）、调音频训练超参或换 backbone。
如果你想加新的评估方式，那是 pet-eval 的事；如果你想加新的数据 ingester，那是 pet-data
的事——pet-train 只对"recipe 给定数据 + 给定模型 → 产出 ModelCard"这一段负责。
...
pet-eval 是流水线中段的"质量决策器"，承担两件耦合的工作：跑 evaluator 算 metrics、
基于 metrics 跑 gate 出 pass/fail 决策。它同时被 pet-train（评估 fine-tuned checkpoint）
和 pet-quantize（评估量化后的 RKLLM 模型）消费——如果评估逻辑分散在这两个仓里，同一个
metric 会有两份实现，标准容易漂移；独立成仓让 metric 定义只有一处、gate threshold 只
读一份 `params.yaml`、`GateCheck` 语义只有一处（pet-schema）。这种"质量真相单一来源"
的设计让 train 阶段和 quantize 阶段的产物可以用同一把尺子量，避免"训出来评估 0.85、
量化完评估 0.83 是因为指标算法不同"这种地狱级歧义。

仓内分两层。第一层在 `src/pet_eval/plugins/`：`vlm_evaluator.py` 跑 fine-tuned VLM 在
gold set 上的端到端预测、`audio_evaluator.py` 走 PANNs zero-shot 推理（F026 retro 之后
默认走新 backend，旧 mobilenetv2 标记 opt-in）、`vision_evaluator.py` 跑分类/检测指标、
`gate.py` 是 gate plugin 实现，读 metrics 应用 threshold 出 GateCheck，
`gate_tiers.py` 装 strict / balanced / permissive 三套预设 threshold（v2.5.0 加的，
让 caller 写 `tier="strict"` 就能切一组阈值）。第二层在 `src/pet_eval/metrics/`：
metric 函数本身（`f1_macro`、`bleu`、`rouge`、`kl_divergence`、`accuracy_per_class`
等）作为最底层不依赖任何 plugin 框架——这让别的仓也能 `from pet_eval.metrics import f1_macro`
直接用。`gold_sets/` 目录管理评估金标——VLM gold 是手工 curated 的图+JSON 配对，
audio gold 是按 5-class taxonomy 分目录的 wav 文件。`fusion/` 目录装着多模态融合评估
（vision + audio 联合决策），rule-based 而非 learned（用户当前业务无 learned fusion
需求）。

进入 pet-eval 最先要建立的认知是 evaluator 和 gate 的解耦——evaluator 只算 metrics
不下结论，gate 拿 metrics 跑阈值出 pass/fail。这种解耦让"换 threshold 不需要重跑
evaluator"。其次是 `apply_gate(card, tier=...)` 的 tier 机制——同一组 metrics 在
strict/balanced/permissive 三套阈值下可能有不同结果，便于 release 不同节点用不同松紧度
（开发期 permissive、上线 strict）。第三是 audio_evaluator 的 backend 切换——`inference_backend="panns"`
是默认值，legacy `mobilenetv2` 是 F026 之前的实现，被发现 broken 后改成显式 opt-in，
确保新 caller 不会无意中走错路径。

来 pet-eval 改代码的典型场景：加一个新 metric（在 `metrics/` 下加纯函数 + 在 evaluator
里调）、加一个新 evaluator plugin（继承 `EvaluatorBase` + 注册 entry-point）、加一个
新 gate tier 预设（在 `gate_tiers.py` 加一行 dict）、调金标集（curate 新样本进 `gold_sets/`
+ 写好 split 文件）、引入新的 fusion 策略。如果你想改 trainer，那是 pet-train 的事；
想改 quantize converter，那是 pet-quantize 的事——pet-eval 只对"checkpoint + 数据 →
metrics + GateCheck"这一段负责。
...
pet-quantize 处理"训练好的浮点 checkpoint → 设备能跑的量化产物"这一段，包括 VLM 用
RKLLM W4A16/W8A8、视觉编码器和音频 CNN 用 RKNN FP16，以及最终把多个 EdgeArtifact 打成
带签名的 tarball。它独立成仓的根本原因是 Rockchip SDK（`rknn-toolkit2` + `rkllm`）是
vendor wheel——不在 PyPI，只能在拿到 Rockchip 账号的工作站装。如果把量化逻辑塞进
pet-train 或 pet-eval，那两个仓在 GitHub Actions ubuntu-latest runner 上将完全装不起
（缺 vendor wheel），整条 CI 被一个 vendor 限制拖死。独立的 pet-quantize 通过
`PET_ALLOW_MISSING_SDK=1` 这个 escape hatch 让 SDK-bound plugin 在无 SDK 环境下被
skip-with-warning 而不是 hard fail——CI 上继续跑 `noop_converter` 和 unit tests，
真量化在有 SDK 的 workstation 上跑。

代码组织围绕"converter plugin + inference runner + packaging"三件事展开。
`src/pet_quantize/plugins/converters/` 装着 4 个 converter：`noop.py`（永远可用，CI 跑
全链路用）、`vision_rknn_fp16.py`（PyTorch → ONNX → RKNN FP16）、`audio_rknn_fp16.py`（音频
CNN → ONNX → RKNN FP16）、`vlm_rkllm_w4a16.py`（HF weights + 校准 batch → RKLLM W4A16），
后三个的 SDK import 都是 lazy（在 `convert/` 子模块的函数体内 import 而不是顶层），
让 plugin 注册时不触发 vendor wheel 要求。`plugins/datasets/` 里是 calibration 数据集
plugin（`vision_calibration_subset` / `audio_calibration_subset`），通过 content-addressable
cache 让相同 calibration 输入产生相同文件名，配合 Orchestrator 的 stage_config_sha
让 resume-from-cache 真的生效。`inference/` 装两个 SDK runner——`rkllm_runner.py` 和
`rknn_runner.py`——支持 PC simulation（无设备时跑 SDK 的 emulator）和 on-device（连
ADB 推到 RK3576）双模，pet-eval 的 `QuantizedVlmEvaluator` 在 CI 中用 simulation 模式
跑完整 lifecycle 而不需要 mock。`packaging/` 提供 `build_package`（tarball + 每文件
SHA-256 manifest）和 `sign_package`（cryptographic signature），是 OTA 的输入。
`validate/quantize_validate/` 是量化质量验证 workflow——量化前后跑同一个 gold set
比较 metrics 偏差，超阈值告警。

新人最先要理解的是"SDK-gated cluster pattern"——converter plugin 和它依赖的 calibration
dataset plugin 总是绑在同一个 try/except ImportError 块里注册，避免出现"有 calibration
dataset 但没有对应 converter"的孤立 plugin。其次是 `RKLLMRunner` 的 dual-mode 设计——
PC simulation 让 CI 能行使完整 init/generate/release 流程而不需要真 NPU。第三是
`packaging.build_package` 写 manifest 时为每个 EdgeArtifact 算 SHA-256——这是 OTA 端
soft-fail 验签链路的起点（pet-ota 那一侧 lazy import `verify_package`，没装 [signing]
extras 时降级 warn-only）。

来 pet-quantize 改代码的典型场景：加一个新 converter（写新 plugin 文件 + 在 `_register.py`
的 cluster try/except 块里注册）、改 packaging manifest 字段（同步动 pet-ota 的
`upload_artifact`）、加一种新的 calibration 子集（继承 DATASETS plugin 接口）、改
`quantize_validate` 的偏差阈值（params.yaml）。如果你想加新的端侧 backend（比如非
Rockchip 的设备），那得新增一整套 SDK-gated cluster；如果你想改 OTA 分发逻辑，那是
pet-ota 的事——pet-quantize 只对"ModelCard.checkpoint_uri → ModelCard.edge_artifacts +
带签名的 tarball"这一段负责。
...
pet-ota 是流水线的终点仓，负责把量化好的制品推送到已售出的喂食器设备。它独立成仓的根本
理由是运行环境的差异：一台部署服务器应该能 `pip install pet-ota` 得到完整的 delta patch +
canary rollout 能力，而不需要装 torch、rknn-toolkit2、LLaMA-Factory 这些训练/量化栈。
pet-ota 因此刻意保持纯 Python（bsdiff4 + pydantic + boto3 三件套），把"轻量"作为 first-class
设计约束。它消费 pet-quantize 的输出（带签名的 tarball + manifest.json），把它推到
storage 后端，然后按 5 状态 FSM（GATE_CHECK → CANARY_DEPLOYING → CANARY_OBSERVING →
FULL_DEPLOYING → DONE，或任一状态走 ROLLING_BACK → ROLLED_BACK 的回滚分支）跑灰度发布，
48 小时观察期内监控失败率，超阈值自动回滚。

仓内有两套看起来名字相似但职责完全不同的 backend，新人最容易混。第一套是
`pet_ota.backend.LocalBackend`（在 `src/pet_ota/backend/`），它是有状态部署编排器——
追踪 `deployments/<id>.json` 文件、追踪 `device_groups/` 里的设备分组、追踪每台设备的
per-device 状态（pending / success / failed / timeout）。FSM 跑的就是这一套，
canary_observe_hours 的 48 小时计时也是它的事。第二套是 `pet_ota.plugins.backends.LocalBackendPlugin`
（在 `src/pet_ota/plugins/backends/`），它是 pet-infra OTA Registry 的 plugin——
接受 `ModelCard` 输入、验证 `gate_status="passed"`、把 EdgeArtifact 复制到 storage
location、写 `manifest.json`、append 一条 `DeploymentStatus` 进 `deployment_history`，
然后把 ModelCard 还回去。两套不能合并，因为合并会把 deployment lifecycle 状态文件、
设备组追踪、per-device 状态都搬进 OTA Registry——有状态的编排逻辑会污染无状态的
plugin 接口。`release/canary_rollout.py` 是 FSM 的真正实现，里面 `canary_rollout()` 函数
是 5 状态机入口，支持 process resume（重启时如果 deployment JSON 已存在，从持久化状态
继续而不是重新 gate_check）。`packaging/upload_artifact.py` 是制品上传 + soft-fail 签名
验证（lazy import pet_quantize 的 verify_package，没装 `[signing]` extras 就 warn-only
跳过）。`monitoring/` 提供 `check_update_rate` + `check_and_alert`，是 canary 观察期
的故障检测入口。

新人最先要建立的认知是 5 状态机的 resume 语义——48 小时观察期里进程可能挂掉重启 N 次，
每次重启不能重新跑 gate_check（gate threshold 可能在期间被改），必须从 deployment JSON
持久化的状态原地继续。其次是"两个 backend"的责任划分——`LocalBackend` 守状态、
`LocalBackendPlugin` 守 Registry 接口契约。第三是 `LocalBackendPlugin.run()` 第一行
那个 `gate_status != "passed"` 抛 `ValueError` 的硬门控——把 gate 不通过的模型推到
设备的可能性堵在 schema 层，比 silent skip 更难悄悄出事。

来 pet-ota 改代码的典型场景：加一个新的 OTA backend（比如 Mender/Balena/S3，写新的
plugin 文件 + 注册）、改 canary 参数（observe_hours / failure_rate_threshold 都从
params.yaml 读，不硬编码）、加一种新的回滚策略、扩展设备分组逻辑。如果你想改量化产物
本身，那是 pet-quantize 的事；如果你想加 release gate 规则（比如"必须有 ≥ N 条 DPO pair
才能 release"），那是 `release/check_gate.py` + params.yaml——和量化无关。
...
pet-id 是 9 仓中唯一与其他 8 仓正交的工具，意图上独立（ecosystem-optimization spec §5.2）：
它的用户是人不是 orchestrator，使用场景是"给这只猫注册一张 PetCard"或"识别照片里是哪
只猫"，而不是"在 ExperimentRecipe 的第 3 个 stage 运行"。它本身不消费任何 pet-* 包
（`PetCard` 在 pet-id 内部定义而不是 pet-schema）、也不向 pet-infra Registry 注册任何
plugin——这种"故意不接入"的姿态是经过设计的：把 pet-id 拉进 peer-dep / plugin 体系会
强制 PetCard 变成 pet-schema 实体（需要 schema 版本管理 + downstream CI 派发），把
`petid` CLI 变成 pet-infra plugin（需要 entry-point + Registry 槽位），这些都是工程
成本，但用户体验上 0 收益，因为用户从来不会通过 orchestrator 调用 pet-id。

仓内代码分两层。底层 `src/purrai_core/`（从 pet-demo bootstrap 来的）是算法核心，
约定靠 `typing.Protocol` 描述（`interfaces/detector.py::Detector` 和
`interfaces/reid.py::ReID`）而不是继承层级，让上层不依赖具体 backend 类。`backends/`
里是真实 ML 实现：`yolov10_detector.py`（猫狗目标检测，给后续 ReID 提供 crop）、
`osnet_reid.py`（512 维 embedding ReID 网络）、`mmpose_pose.py`（姿态估计辅助）、
`bytetrack_tracker.py`（多帧 ID 追踪）、`qwen2vl_narrative.py`（生成"这是一只橘猫，
正在吃饭"这类自然语言描述）。这些 backend 必须 import 真实库不做 mock 替换（pet-demo
bootstrap 时立的规矩）。上层 `src/pet_id_registry/` 是面向用户的 CLI 与磁盘 gallery
层：`card.py` 定义 `PetCard` Pydantic 模型 + `compute_pet_id()` 内容寻址 ID 函数、
`library.py` 实现磁盘 PetCard 增删查改 + cosine similarity identify、`enroll.py`
处理 photo / dir / video 三种注册流程、`cli.py` 是 Click 命令入口 (`petid register`,
`petid identify`, `petid list`, `petid delete`)。

新人最先要理解的是 `compute_pet_id()` 那两个不可省略的步骤：little-endian 字节序锁定
（`<f4`）保证跨 amd64/arm64 主机产生相同 ID、L2-normalize 断言保证同一只猫不同光照下
embedding 模长一致从而 ID 一致。第二是 `Library.save()` 的 `os.replace()` 原子目录 swap——
PetCard 包含 card.json + cover.jpg + views/*.npy + views/*.jpg 多个文件，写到 .tmp 子
目录拼装好再原子 rename 到目标位置，中途 SIGKILL 不会留半成品。第三是 purrai_core 与
pet_id_registry 的两层分离——前者是算法可被其他 CLI 复用的部分，后者是带磁盘 gallery
的具体 CLI，分离让测试时可以 `patch()` 在 Protocol boundary 而不必装所有 ML 库。

来 pet-id 改代码的典型场景：加新的 detector/ReID backend（在 backends/ 加文件 + Protocol
约束）、加新的 CLI 子命令（动 `cli.py` + 对应 enroll/library 函数）、改 PetCard 字段
（动 `card.py` + 注意 `extra="forbid"` 同样守着新增字段必须显式声明）、改 identify
threshold（params.yaml）。如果你的需求是"在 train pipeline 里识别这只猫然后做 per-pet
fine-tune"——这是另一个故事，目前 pet-id 不为 train pipeline 提供 API。
...
Plugin 是这个生态实现"thin orchestrator + sibling 仓贡献业务"赌注的具体机制。pet-infra
提供 7 个 mmengine `Registry` 实例（TRAINERS / EVALUATORS / CONVERTERS / DATASETS /
STORAGE / OTA / METRICS）作为中央存放点；sibling 仓把自己的实现类用 `@Registry.register_module(name=...)`
装饰器注册进去；启动时由 `discover_plugins()` 通过 `importlib.metadata.entry_points("pet_infra.plugins")`
扫描所有装好的 pet-* 包，调它们的 `register_all()` 函数让注册装饰器执行；之后
Orchestrator 只需要按 `recipe.stages[i].plugin_name` 去对应 Registry 取类、按 config
实例化、调 `.run(input_card, recipe)`。

这一节先列出 7 个 Registry 的 slot 划分（哪个仓负责贡献哪些 plugin），然后讲 entry-point
discovery 的懒加载策略和它带来的契约（`pet_run()` 调用前必须先 `discover_plugins()`），
再用一张时序图展示从 `pet validate` CLI 到 plugin 实际进 Registry 的完整调用链——这条
链是 newcomer 调试 "plugin 找不到" 问题时最有用的一张图。最后两节谈 mmengine.Registry
本身的限制（按字符串名查、不支持子类层级路由）以及为什么这些限制对当前业务可接受，
还有 plugin contract 的测试纪律——DEV_GUIDE §11.8 的 fixture-real 要求是从 11 次同款
retro 蒸馏出来的硬规则（F008/F011/F012/F014/F021-F027 全部是"unit-test mock-only +
没真 path 跑过"），凡是动 plugin contract 层的 PR 必须勾"fixture-real test added"box，
没勾就拒。
...
跨仓约定是这个 monorepo 的"宪法"——9 个仓彼此独立但要协同运转，只能靠几条所有仓都遵守
的硬规则来维持秩序。这一节讲三件事：依赖管理（β peer-dep 模式）、版本真相（compatibility
matrix）、协作流程（dev → main 分支规范）。它们彼此关联——peer-dep 让消费方掌控装哪个
版本、matrix 把"该装哪个版本"写在一处、分支规范让矩阵更新和代码改动绑定可审计。读这节
最有效的方式不是死记每条规则，而是理解它们各自解决的具体痛点：peer-dep 解决"pet-data
要 schema v3.0、pet-train 要 v3.3"的依赖冲突地狱；matrix 解决"哪个 pet-train tag
该配哪个 pet-schema tag"的人肉记忆负担；分支规范解决"feature 分支直接 PR 进 main 让
集成验证缺位"的过去事故。
...
CI/CD 在这个 monorepo 里承担的角色比单仓项目里更重——9 个独立仓库各跑各的 pipeline，
但彼此契约耦合（pet-schema 一改 6 个下游仓必须同步），仅靠 reviewer 人眼检查不可能不
漏掉跨仓影响。这一节介绍每个 workflow 的角色（按"防什么 bug"组织），重点讲两个最能
体现 retro 教训的 workflow——`release-version-consistency.yml` 和
`cross-repo-smoke-install.yml`。这两个不是凭空设计的，前者是 F018（5 个 release tag
ship 出来 `importlib.metadata.version()` 返回旧版本号）的 prevention guard，后者是
"装序约定文档化但从来没真试过装一遍"这个隐患的端到端兜底。整体设计原则：每条 CI 守护
一类已经发生过的事故，不为想象中的事故添加 workflow（它们会变成噪音）。
...
这一节是上面所有概念的落地——把 §3 各仓的 plugin、§4 的 Registry 机制、§5 的 peer-dep
装序串成可复制粘贴的命令链。涵盖四个最常被问的真实场景：跑一次完整 SFT 训练加评估
round-trip（验证 train + eval 两段都能跑通的最小有效路径）、用 replay 验证可复现性
（含 sha256 fail-fast 和 git_shas drift warn 两条路径）、跑 cross-modal fusion 评估
sweep（演示 fusion plugin 的 weighted 决策）、添加一个新的 trainer plugin（端到端示例
从写代码到 entry-point 注册到被 Orchestrator 调用）。这里假设你已经按入门指南装好了
共享 conda 环境和按 matrix row 的版本装好了 pet-* 包；如果还没，回到入门指南的"5 分钟
跑通一次"那一节先把基础环境跑起来。每个示例都包含期望输出片段，跑出来不一致基本等于
找到了一个 finding——这种"按 doc 跑不出 doc 说的结果"是 ecosystem 漂移最早的告警信号。
