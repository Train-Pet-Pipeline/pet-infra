...
class VisionAnnotation(BaseAnnotation):
    modality: Literal["vision"] = "vision"
    raw_response: str
    parsed: PetFeederEvent                # 现有 VLM 结构化输出保留
    prompt_hash: str
...
**交付**：
- Alembic migration 增加 `modality` / `storage_uri` 列
- `FrameRecord` 迁移为 `VisionSample`；新增 `AudioSample` 建表
- AnnotationRecord 拆 VisionAnnotation / AudioAnnotation
...
**风险**：DB migration 失败需 dump 恢复；LS 模板同步
